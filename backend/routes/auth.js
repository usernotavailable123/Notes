...
router.post(
  ...
  [
    ...
    body("password", "Password must be atleast of length 5").isLength({
      min: 5,
    }),
  ],
  async (req, res) => {
    ...
    try {
      ...
      if (user) {
        ...
        return res
          .status(400)
          .json({ error: "User with this email already exists",success });
      }
...
router.post("/getuser",fetchusers,async (req, res) => {
    try {
        userId = req.user.id;
        const user = await User.findById(userId).select("-password"); //take all the data of the corresponding userId except password
        res.send(user);
    } catch (error) {
        console.log(error.message);
        res.status(500).send("Internal Server Error");
